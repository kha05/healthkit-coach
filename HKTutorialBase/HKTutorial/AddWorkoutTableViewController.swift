...
class AddWorkoutTableViewController: UITableViewController {
  ...
  func datetimeWithDate(_ date:Date , time:Date) -> Date? {
    ...
    let dateComponents = currentCalendar.components(.CalendarUnitDay | .CalendarUnitMonth | .CalendarUnitYear, fromDate: date)
    let hourComponents = currentCalendar.components(.CalendarUnitHour | .CalendarUnitMinute, fromDate: time)
    
    let dateWithTime = currentCalendar.dateByAddingComponents(hourComponents, toDate:currentCalendar.dateFromComponents(dateComponents)!, options:NSCalendar.Options(0))
